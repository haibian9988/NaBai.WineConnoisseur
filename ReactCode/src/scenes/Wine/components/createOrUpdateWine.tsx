...
import rules from './createOrUpdateWine.validation';

export interface ICreateOrUpdateWineProps extends FormComponentProps {
  visible: boolean;
  modalType: string;
  onCreate: () => void;
  onCancel: () => void;
...
class CreateOrUpdateWine extends React.Component<ICreateOrUpdateWineProps> {
  render() {
    ...
    return (
      <Modal visible={visible} onCancel={onCancel} onOk={onCreate} title={L('Wines')} width={550}>
        <Form>
...
export default Form.create<ICreateOrUpdateWineProps>()(CreateOrUpdateWine);
